...
async function analyzePdf(filePath) {
  ...
  for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
    ...
    Object.entries(markers).forEach(([marker, y]) => {
    ...
    // Split into regions
    const labelZone = items.filter(i => (i.transform?.[5] || 0) > maxY - 400);
    const invoiceZone = items.filter(i => (i.transform?.[5] || 0) <= maxY - 400);
    
    console.log(`    Label zone (bottom ~400pt): ${labelZone.length} items`);
    console.log(`    Invoice zone (top): ${invoiceZone.length} items`);
    
    // Sample text from each region
    const labelText = labelZone.slice(0, 15).map(i => (i.str || '').trim()).filter(s => s).join(' ');
    const invoiceText = invoiceZone.slice(0, 15).map(i => (i.str || '').trim()).filter(s => s).join(' ');
    
    console.log(`    Label zone sample: ${labelText.substring(0, 80)}`);
    console.log(`    Invoice zone sample: ${invoiceText.substring(0, 80)}`);
  }
}
